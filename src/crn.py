...
def generate_base_demand(n_hours: int = N_HOURS) -> np.ndarray:
    """
    Deterministic base demand profile (MW), normalized so that
    max(D_base) = PEAK_DEMAND_MW.

    Raw shape: base + seasonal*sin(...) + diurnal*sin(...)
    Then scaled so the profile peak equals PEAK_DEMAND_MW, preserving
    load factor ~ 0.60 and realistic seasonal/diurnal structure.
    ...
    seasonal = p["seasonal_amplitude"] * np.sin(2 * np.pi * t / 8760 - np.pi / 2)
    diurnal = p["diurnal_amplitude"] * np.sin(2 * np.pi * (t % 24) / 24 - np.pi / 3)

    raw = p["base_fraction"] + seasonal + diurnal
    # Normalize so max = 1.0, then scale to PEAK_DEMAND_MW
...
def generate_common_inputs(
    ...
    if renewable_mode == "stochastic":
        wind_cf = rng_renew.beta(*WIND_BETA_PARAMS, size=n_hours)
        solar_cloud = rng_renew.beta(*SOLAR_BETA_PARAMS, size=n_hours)
